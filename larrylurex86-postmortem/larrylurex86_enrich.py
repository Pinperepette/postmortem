...
SRC = "/Users/pinperepette/Porgetti/tool-idea/larrylurex86-postmortem/larrylurex86_data.json"
...
# ═══════════════════════════════════════════════════════════════════════════
# 15. STRATEGY BLOCK (data-driven)
# ═══════════════════════════════════════════════════════════════════════════
SCREEN = data["profile"]["screen_name"]

# audit
recent_cutoff = (last_post_dt - _td(days=365)).date()
recent_posts = [p for p in posts if _dt.strptime(p["dt"], "%Y-%m-%d").date() >= recent_cutoff]
posts_recent_12m = len(recent_posts)
days_active = max((last_post_dt.date() - _dt.strptime(recent_posts[0]["dt"], "%Y-%m-%d").date()).days, 1) if recent_posts else 1
cadence_per_week = round(posts_recent_12m / days_active * 7, 2)

recent_views = [p["views"] for p in recent_posts]
median_views_recent = sorted([v for v in recent_views if v > 0])[len([v for v in recent_views if v > 0])//2] if any(v>0 for v in recent_views) else 0
reach_pct = round(median_views_recent / followers_now * 100, 1) if followers_now else 0
zero_views_recent_pct = round(sum(1 for v in recent_views if v == 0) / len(recent_views) * 100, 1) if recent_views else 0
reply_pct_recent = round(sum(1 for p in recent_posts if p["is_reply"]) / len(recent_posts) * 100, 1) if recent_posts else 0

audit = {
    "followers": followers_now,
    "posts_recent_12m": posts_recent_12m,
    "cadence_per_week": cadence_per_week,
    "median_views_recent": median_views_recent,
    "reach_pct": reach_pct,
    "zero_views_pct": zero_views_recent_pct,
    "reply_pct_recent": reply_pct_recent,
    "historic_zero_views_pct": data["kpis"]["zero_views_pct"],
    "historic_reply_pct": data["diagnostics"]["reply_share_pct"],
    "gini": round(gini_views, 3),
    "growth_score": growth_score,
}

# top_formats: media × topic primario, almeno 3 post
fmt_buckets = defaultdict(list)
for p in posts:
    primary_topic = next((t for t in p["topics"] if t != "Altro"), "Altro")
    fmt_buckets[(p["media_type"], primary_topic)].append(p)
top_formats = []
for (media, topic), pts in fmt_buckets.items():
    if len(pts) < 3:
        continue
    pts_sorted = sorted(pts, key=lambda x: -x["views"])
    avg_v = round(sum(p["views"] for p in pts) / len(pts))
    best = pts_sorted[0]
    top_formats.append({
        "fmt": f"{media} · {topic}",
        "posts": len(pts),
        "avg_views": avg_v,
        "top_text": best["text"][:140],
        "top_url": best.get("post_url"),
    })
top_formats.sort(key=lambda x: -x["avg_views"])
top_formats = top_formats[:8]

# best_slots: heatmap cells con almeno 2 post, sopra mediana
overall_avg = data["kpis"]["avg_views"]
day_names = ["Lun","Mar","Mer","Gio","Ven","Sab","Dom"]
best_slots = []
for cell in heatmap:
    if cell["posts"] >= 2 and cell["avg_views"] > overall_avg:
        best_slots.append({
            "day": day_names[cell["wd"]],
            "hour": cell["h"],
            "posts": cell["posts"],
            "avg_views": cell["avg_views"],
        })
best_slots.sort(key=lambda x: -x["avg_views"])
best_slots = best_slots[:6]

# templates: top 8 post organici
templates = []
for p in sorted(posts, key=lambda x: -x["views"])[:8]:
    templates.append({
        "dt": p["dt"],
        "text": p["text"][:200],
        "media": p["media_type"],
...
# top_rt_authors: dal blocco rt, escludendo self-RT
top_rt_authors = [a for a in data["rt"]["top_authors"] if a["author"].lower() != SCREEN.lower()][:8]

# mentioned: estrai @-mentions dal testo
mention_re = re.compile(r"@(\w+)")
mention_counter = Counter()
for p in posts:
    for m in mention_re.findall(p["text"] or ""):
        if m.lower() != SCREEN.lower():
            mention_counter[m] += 1
mentioned = [{"handle": h, "count": c} for h, c in mention_counter.most_common(10)]

# topic_score: per ogni topic, post con views > 0, avg, mediana
topic_score = []
for t in data["topics"]:
    if t["topic"] == "Altro":
        continue
    tposts = [p for p in posts if t["topic"] in p["topics"]]
    pwv = [p for p in tposts if p["views"] > 0]
    if not pwv:
        continue
    sorted_v = sorted([p["views"] for p in pwv])
    med = sorted_v[len(sorted_v)//2]
    topic_score.append({
        "topic": t["topic"],
        "posts_with_views": len(pwv),
        "avg_views": round(sum(p["views"] for p in pwv) / len(pwv)),
        "median_views": med,
    })
topic_score.sort(key=lambda x: -x["avg_views"])
topic_score = topic_score[:7]

# suggested_week: combina best_slots + top_formats
suggested_week = []
fmt_idx = 0
for slot in best_slots[:5]:
    fmt = top_formats[fmt_idx % max(len(top_formats), 1)] if top_formats else {"fmt": "post", "top_text": "", "avg_views": 0}
    fmt_idx += 1
    suggested_week.append({
        "day": slot["day"],
        "hour": slot["hour"],
        "type": fmt["fmt"],
        "why": f"Slot {slot['day']} h{slot['hour']} = {slot['avg_views']:,} avg views (n={slot['posts']}); format {fmt['fmt']} = {fmt['avg_views']:,} avg",
        "sample": (fmt.get("top_text") or "")[:120],
    })

# diagnosis: livelli derivati dai numeri
diag = []
if cadence_per_week > 30:
    diag.append({
        "level": "warn",
        "title": f"Cadenza: {cadence_per_week} post/settimana",
        "body": f"Stai postando troppo ({posts_recent_12m} post in 12 mesi). Sotto i {median_views_recent} views mediani: l'algoritmo non distingue i post 'forti' dal rumore. Ridurre a 5-10/settimana e investire nei post che funzionano.",
    })
elif cadence_per_week < 3:
    diag.append({
        "level": "critical",
        "title": f"Cadenza: {cadence_per_week} post/settimana",
        "body": f"Solo {posts_recent_12m} post in 12 mesi. X premia consistenza: sotto i 3-5 post/settimana l'algoritmo ti tratta come account dormiente.",
    })
else:
    diag.append({
        "level": "good",
        "title": f"Cadenza: {cadence_per_week} post/settimana",
        "body": f"Ritmo sostenibile ({posts_recent_12m} post in 12 mesi).",
    })

if reach_pct < 5:
    diag.append({
        "level": "critical",
        "title": f"Reach mediano: {reach_pct}% dei follower",
        "body": f"Mediana {median_views_recent} views su {followers_now:,} follower. L'algoritmo non sta distribuendo il contenuto fuori dal tuo network — segnale di basso engagement-rate o topic dispersi.",
    })
elif reach_pct < 15:
    diag.append({
        "level": "warn",
        "title": f"Reach mediano: {reach_pct}% dei follower",
        "body": f"Mediana {median_views_recent} views su {followers_now:,} follower — sotto la soglia tipica del 10-20% per account 'sani'.",
    })

if data["kpis"]["zero_likes_pct"] > 50:
    diag.append({
        "level": "warn",
        "title": f"Zero-likes: {data['kpis']['zero_likes_pct']}% dei post",
        "body": f"Più di metà dei post non riceve nemmeno un like. Segnale di audience disallineata o contenuto poco distintivo.",
    })

if normalized_entropy > 0.75:
    diag.append({
        "level": "warn",
        "title": f"Entropia topic: {normalized_entropy}",
        "body": "Topic troppo dispersi: l'algoritmo non riesce a collocarti in un cluster di interesse. Risultato: peggiora la raccomandazione fuori dal tuo network.",
    })

# headline_html: data-driven
top_topic_name = topic_score[0]["topic"] if topic_score else "vario"
if cadence_per_week > 30 and reach_pct < 10:
    headline_html = (
        f"Posti molto ({posts_recent_12m} post negli ultimi 12 mesi, ~{round(cadence_per_week)} a settimana) "
        f"ma la mediana è ferma a <strong>{median_views_recent} views</strong> su {followers_now:,} follower "
        f"(reach <strong>{reach_pct}%</strong>). Non è un problema di volume: è un problema di "
        f"<strong>focus</strong> — entropia topic {normalized_entropy} significa che l'algoritmo non capisce di cosa parli."
    )
elif cadence_per_week < 3:
    headline_html = (
        f"Account quasi dormiente: solo {posts_recent_12m} post in 12 mesi. "
        f"Top post storico: <strong>{data['kpis']['top_post_views']:,} views</strong> — la base c'è, manca la cadenza."
    )
else:
    headline_html = (
        f"{posts_recent_12m} post in 12 mesi · cadenza {cadence_per_week}/sett · "
        f"mediana <strong>{median_views_recent} views</strong> su {followers_now:,} follower. "
        f"Topic top per views: <strong>{top_topic_name}</strong>."
    )

# targets_30d: dipende dallo stato
if cadence_per_week > 30:
    cad_target = "ridurre a 5-10 post/sett (qualità)"
elif cadence_per_week < 3:
    cad_target = "salire a 5-7 post/sett"
else:
    cad_target = f"mantenere ~{round(cadence_per_week)}/sett"

reach_target = f"mediana > {max(int(median_views_recent * 1.5), 100)} views"
eng_target = f"ER medio > {max(round(data['kpis']['avg_engagement_rate'] * 1.5, 1), 1.0)}%"

if reply_pct_recent < 20:
    reply_target = "20% dei post = reply ad account più grandi"
else:
    reply_target = f"mantenere reply ~{int(reply_pct_recent)}%"

targets_30d = {
    "cadence": cad_target,
    "reach": reach_target,
    "engagement": eng_target,
    "reply_strategy": reply_target,
}
...
data["strategy"] = {
    ...
    "templates": templates,
    "top_rt_authors": top_rt_authors,
    "mentioned": mentioned,
    "topic_score": topic_score,
    "suggested_week": suggested_week,
    "diagnosis": diag,
    "targets_30d": targets_30d,
}
